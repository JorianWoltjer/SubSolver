...
use sub_solver::{load_wordlist, normalize, input::{clean_input, input_to_words}, solve::{Solver, Solution}};
...
fn load_wordlist_bench(c: &mut Criterion) {
    ...
    c.bench_function("load_wordlist", |b| b.iter(|| load_wordlist(black_box(&wordlist))));
}
...
fn input_to_words_bench(c: &mut Criterion) {
    ...
    c.bench_function("input_to_words", |b| b.iter(|| input_to_words(black_box(input), black_box(&dictionary))));
}
...
fn solve_bench(c: &mut Criterion) {
    ...
    c.bench_function("solve", |b| b.iter(|| Solver::new(&cipher_words).solve(HashMap::new(), None)));
}

fn apply_solution_bench(c: &mut Criterion) {
    let ciphertext = "x cbt tloap";
    let solution = Solution {
        key: [('x', 'a'), ('c', 'f'), ('b', 'e'), ('t', 'w'), ('l', 'o'), ('o', 'r'), ('a', 'd'), ('p', 's')].iter().cloned().collect(),
    };
    
    c.bench_function("apply_solution", |b| b.iter(|| solution.apply(black_box(ciphertext))));
}

criterion_group!(benches, load_wordlist_bench, normalize_bench, clean_input_bench, input_to_words_bench, solve_bench, apply_solution_bench);
